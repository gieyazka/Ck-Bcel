...
const { MCID, MCC, SHOPCODE, CCY, COUNTRY, PROVINCE, SERECT_KEY ,JWT_SECRET} = process.env;
app.use(cors());
  ...
  onePay.subscribe(subParams,async (res) => {
    ...
    if(res.uuid){
      ...
      if(type ==="buyLottery"){
        ...
        await axios.get(`${process.env.CK_BACKEND}/api/summary?payload=${callbackUrl}`);
